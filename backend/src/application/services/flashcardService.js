...
class FlashcardService {
  async getFlashcardsByDeck(deckId, userId) {
    ...
    if (!deck) {
      throw new Error(
        "Không tìm thấy bộ bài hoặc bộ bài này đang ở chế độ riêng tư!",
      );
    }
  ...
  async createFlashcard(userId, cardDto) {
    ...
    if (!deck) {
      throw new Error(
        "Không tìm thấy bộ bài hoặc bạn không có quyền thêm thẻ vào đây!",
      );
    }

    const newCard = {
      _id: cardDto.id || uuidv4(),
  ...
  async updateFlashcard(cardId, userId, updateDto) {
    ...
    if (updateDto.interval !== undefined)
      updateData.interval = updateDto.interval;
    if (updateDto.repetition !== undefined)
      updateData.repetition = updateDto.repetition;
    if (updateDto.easeFactor !== undefined)
      updateData.ease_factor = updateDto.easeFactor;
    if (updateDto.nextReviewDate)
      updateData.next_review_date = updateDto.nextReviewDate;

    const updatedCard = await flashcardRepository.updateById(
      cardId,
      updateData,
    );
  ...
  async restoreFlashcard(cardId, userId) {
    ...
    if (!restoredCard)
      throw new Error("Thẻ không nằm trong thùng rác hoặc không tồn tại");
